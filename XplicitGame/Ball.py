...
class Ball(GameObject):
    ...
    def changeDirection(self, gameObject):
        ...
        if position[1] > objectPosition[1] and position[1] < objectPosition[1] + objectSize[1] and position[0] > objectPosition[0] and position[0] < objectPosition[0] + objectSize[0]:
            self.setPosition((position[0], objectPosition[1] + objectSize[1]))
            # invert direction to Y-axis
            self.__direction[1] *= -1

        elif position[1] + size[1] > objectPosition[1] and position[1] + size[1] < objectPosition[1] + objectSize[1] and position[0] < objectSize[0] and position[0] < objectPosition[0] + objectSize[1]:

            self.setPosition((position[0], objectPosition[1] - objectSize[1]))
            # invert direction to Y-axis
            self.__direction[1] *= -1

        elif position[0] + size[0] > objectPosition[0] and position[0] + size[0] < objectPosition[0] + objectSize[0]:

            self.setPosition((objectPosition[0] - size[0], position[1]))
            # invert direction to X-axis
            self.__direction[0] *= -1

        else:

            self.setPosition((objectPosition[0] + objectSize[0], position[1]))
            # invert direction to X-axis and Y-axis
            self.__direction[0] *= -1
            self.__direction[1] *= -1
    ...
    def updatePosition(self):
        ...
        # check if one is outside the bounds of the app to right-hand side
